...
minimap2
```{r}
soupc_all <- list.files("data/processed/DB52e_star/", pattern = "clusters.tsv", full.names = T, recursive = T, include.dirs = T)%>% set_names(nm = (str_remove_all(.,'data/processed/DB52e_star//|soupc/mi.*k|/clusters.tsv') %>% str_replace(., '/', '_'))) %>% .[str_to_lower(sorted_names)]%>%
...
soupc_all <- list.files("data/processed/DB52e_star/", pattern = "clusters.tsv", full.names = T, recursive = T, include.dirs = T)%>%
  set_names(nm = (str_remove_all(.,'data/processed/DB52e_star//|soupc/h.*k|/k|/clusters.tsv') %>% str_replace_all(., '/', '_'))) %>% .[str_to_lower(sorted_names)] %>%
...
combi
```{r}
soupc_all_al <- list.files("data/processed/DB52e_star/", pattern = "clusters.tsv", full.names = T, recursive = T, include.dirs = T)%>% set_names(nm = (str_remove_all(.,'data/processed/DB52e_star//|soupc/|k|/clusters.tsv') %>% str_replace_all(., '/', '_')))  %>% .[str_to_lower(sorted_names_al)]%>%
  map_df(read_delim, delim = "\t", col_types = 'cccddd', .id = "sample_k") 
...
##!!!!!!NB - TO BE DELETED IN FUTURE WITH THE msc NAMES IN PATHS HAVE BEEN CAPITALIZED
names(soupo_kselect_list) <- str_to_upper(names(soupo_kselect_list))
...
##!!!!!!NB - TO BE DELETED IN FUTURE WITH THE msc NAMES IN PATHS HAVE BEEN CAPITALIZED
names(soupo_PC_mtx) <- str_to_upper(names(soupo_PC_mtx))
